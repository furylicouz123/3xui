...
Для автоматизированной установки без интерактивного ввода используйте **Способ 1**:
```bash
curl -sSL https://raw.githubusercontent.com/furylicouzzz123/3xui-full/main/install.sh | bash -s example.com
```

Это особенно полезно для:
- Автоматизированных скриптов развертывания
- CI/CD пайплайнов
- Массовой установки на несколько серверов
- Избежания ошибок при интерактивном вводе
...
├── cleanup.sh             # Скрипт очистки (скачивается автоматически)
├── quick-deploy.sh        # Скрипт быстрого развертывания (скачивается автоматически)
├── Makefile              # Команды для управления (скачивается автоматически)
├── index.html            # Главная страница (скачивается автоматически)
├── README.md             # Документация (скачивается автоматически)
├── README-Docker.md      # Docker документация (скачивается автоматически)
├── QUICK-START.md        # Быстрый старт (скачивается автоматически)
├── DEPLOY-INSTRUCTIONS.md # Инструкции развертывания (скачивается автоматически)
├── CLEANUP-GUIDE.md      # Руководство по очистке (скачивается автоматически)
├── TROUBLESHOOTING.md    # Решение проблем (скачивается автоматически)
├── .gitignore            # Git ignore файл (скачивается автоматически)
├── website/              # Файлы веб-сайта (скачиваются автоматически)
│   ├── index.html
│   ├── script.js
│   └── assets/
│       ├── main.png
│       └── gwafkIWvPelPtFvgTfDuJjXOKxc.mp4
├── ssl/                  # SSL сертификаты
├── 3x-ui/               # Данные 3X-UI
└── logs/                # Логи сервисов
```

**Автоматическое скачивание:** При запуске `install.sh` все файлы проекта автоматически скачиваются из репозитория GitHub, включая конфигурации, скрипты, документацию и веб-сайт. Больше не нужно клонировать репозиторий вручную!
...
### Ошибка "Домен не может быть пустым!"
**Решение:** Используйте один из способов передачи домена:
```bash
# Способ 1: Параметр командной строки
curl -sSL https://raw.githubusercontent.com/furylicouzzz123/3xui-full/main/install.sh | bash -s your-domain.com
...
### Проблемы с скачиванием файлов
**Симптомы:** Ошибки "файл не найден" или "не удалось скачать"
**Решение:**
```bash
